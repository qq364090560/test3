    ...
    <el-card class="right-card">
      <div v-if="editForm.pname">
        ...
        <el-form class="editForm" ref="editForm" :model="editForm" :rules="rules" label-width="120px">
            ...
            <h4>非机动车</h4>
            <el-collapse style="padding-left:10px" accordion v-for="(it,index) in editForm.nonMotorFeeRules" :key="index + 'a'">
              <el-collapse-item :title="it.feeName">
                <h3 style="text-align:center">包月规则</h3>
                <el-row :gutter="20" v-if="it.monthlyFee">
                ...
                <el-row :gutter="20" v-if="it.tempFee">
                  <el-col :span="12">临停费用:{{it.tempFee.tempFree }}</el-col>
                  <el-col :span="12">开门次数:{{it.tempFee.openCount }}</el-col>
                  <el-col :span="12">临停时间:{{it.tempFee.tempHour }}</el-col>
                </el-row>
                <div>
                  <el-button type="primary" size="mini" @click="editRule(it)">编辑</el-button>
                  <el-button type="danger" size="mini" @click="delRule(it)">删除</el-button>
                </div>
              </el-collapse-item>
            </el-collapse>
        </el-form> 
